...
//        autoBindLifecycle(context);
    ...
    private fun count(time: Long, offset: Long, timeUnit: TimeUnit, isCountDown: Boolean) {
        ...
        mCountDownTimer = object : CountDownTimer(millisInFuture, interval) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                text = if (TextUtils.isEmpty(mCountDownText)) {
                    showTime
                } else String.format(mCountDownText!!, showTime)
    ...
    companion object {
        ...
        fun generateTime(time: Long): String {
            ...
            format = if (day > 0) {
                String.format("%d天%02d:%02d:%02d", day, hours, minutes, seconds)
            } else if (hours > 0) {
                String.format("%02d:%02d:%02d", hours, minutes, seconds)
            } else if (minutes > 0) {
                String.format("00:%02d:%02d", minutes, seconds)
            } else {
                String.format("00:00:%2d", seconds)
            }
            return format
        }
    }
